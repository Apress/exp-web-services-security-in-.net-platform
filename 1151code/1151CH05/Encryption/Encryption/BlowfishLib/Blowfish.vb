...
'See http://www.counterpane.com/bfsverlag.html for details of Algorithm
Public Class Blowfish

    Public Function CreateEncryptor() As ICryptoTransform
        Return New BlowfishCrypto(Me, True)
    End Function

    Public Function CreateDecryptor() As ICryptoTransform
        Return New BlowfishCrypto(Me, False)
    End Function
    ...
    Public Sub New(ByVal key() As Int32)
        ...
        For i = 0 To 17
            _PArray(i) = _PArray(i) Xor key(tmp)
            tmp = (tmp + 1) Mod key.Length
        Next
        ...
        For i = 0 To 16 Step 2
            _PArray(i) = ExtractHighOrder(tmp64)
            _PArray(i + 1) = ExtractLowOrder(tmp64)
        ...
        For i = 0 To 3
            For j = 0 To 254 Step 2
                _SBoxes(i, j) = ExtractHighOrder(tmp64)
                _SBoxes(i, j + 1) = ExtractLowOrder(tmp64)
                tmp64 = Encrypt(tmp64)
            Next
        Next
    ...
    Public Function Encrypt(ByVal x As Int64) As Int64
        Dim xL, xR, tmp As Int32

        'Divide x into two 32-bit halves: xL, xR
        xL = ExtractHighOrder(x)
        xR = ExtractLowOrder(x)
        ...
        'xR = xR Xor P17
        xR = xR Xor _PArray(16)

        'xL = xL Xor P18
        xL = xL Xor _PArray(17)

        'Recombine(xL And xR)
        Return Combine(xL, xR)
    End Function

    Public Function Decrypt(ByVal x As Int64) As Int64
        Dim xL, xR, tmp As Int32
    ...
    Private Function F(ByVal xL As Int32)
        ...
        Return ((_SBoxes(0, a) + _SBoxes(1, b) Mod 232) Xor _SBoxes(2, c)) + _SBoxes(3, d) Mod 232
    End Function

    '1. The P-array consists of 18 32-bit subkeys:
    'P1, P2,..., P18.
    Private _PArray() As Int32 = _
        {&H243F6A88, &H85A308D3, &H13198A2E, &H3707344, &HA4093822, &H299F31D0, _
    ...
    'S3,0, S3,1,..., S3,255;
    'S4,0, S4,1,..,, S4,255.
    Private _SBoxes(,) As Int32 = _
        {{&HD1310BA6, &H98DFB5AC, &H2FFD72DB, &HD01ADFB7, &HB8E1AFED, &H6A267E96, _
            ...
            &H53B02D5D, &HA99F8FA1, &H8BA4799, &H6E85076A}, _
         {&H4B7A70E9, &HB5B32944, &HDB75092E, &HC4192623, &HAD6EA6B0, &H49A7DF7D, _
            ...
            &H71DFF89E, &H10314E55, _
            &H81AC77D6, &H5F11199B, &H43556F1, &HD7A3C76B, &H3C11183B, &H5924A509, _
            &HF28FE6ED, &H97F1FBFA, &H9EBABF2C, &H1E153C6E, &H86E34570, &HEAE96FB1, _
            &H860E5E0A, &H5A3E2AB3, &H771FE71C, &H4E3D06FA, &H2965DCB9, &H99E71D0F, _
            &H803E89D6, &H5266C825, &H2E4CC978, &H9C10B36A, &HC6150EBA, &H94E2EA78, _
            &HA5FC3C53, &H1E0A2DF4, &HF2F74EA7, &H361D2B3D, &H1939260F, &H19C27960, _
            &H5223A708, &HF71312B6, &HEBADFE6E, &HEAC31F66, &HE3BC4595, &HA67BC883, _
            &HB17F37D1, &H18CFF28, &HC332DDEF, &HBE6C5AA5, &H65582185, &H68AB9802, _
            &HEECEA50F, &HDB2F953B, &H2AEF7DAD, &H5B6E2F84, &H1521B628, &H29076170, _
            &HECDD4775, &H619F1510, &H13CCA830, &HEB61BD96, &H334FE1E, &HAA0363CF, _
            &HB5735C90, &H4C70A239, &HD59E9E0B, &HCBAADE14, &HEECC86BC, &H60622CA7, _
